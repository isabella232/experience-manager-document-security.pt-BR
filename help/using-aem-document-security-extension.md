---
title: Usando AEM Extensão de Segurança de Documento para Microsoft&reg; Escritório
...
# Uso AEM extensão de segurança de documento para o Microsoft® Office{#using-aem-document-security-extension-for-microsoft-office}
...
Usando a Extensão de Segurança de Documento para o Microsoft® Office, você pode executar as seguintes tarefas:
...
* Abra as páginas da Web Segurança de documento para exibir o histórico de auditoria do arquivo

### Conexão com um servidor de Segurança de documentos {#connect-to-a-document-security-server}

Se sua intenção for aplicar políticas a arquivos, é preciso configurar as definições de conexão para a Segurança de documentos. Dependendo de como a extensão de segurança de documentos para o Microsoft® Office foi instalada, talvez você já tenha as configurações de conexão padrão. Você pode adicionar configurações de conexão para uma ou mais instâncias da Segurança de documentos. E pode também obter informações do servidor do administrador de Segurança de documentos.

Defina o servidor que deseja usar para proteger arquivos ou gerenciar seus arquivos protegidos como o servidor padrão. Ao aplicar uma política a um novo arquivo ou abrir as páginas da Web da Segurança de documento, a Extensão de segurança de documento para o Microsoft® Office se conecta ao servidor padrão. Se você proteger arquivos usando mais de uma instância de Segurança de documentos, deverá alterar a configuração padrão do servidor ao alternar entre servidores. É possível abrir arquivos protegidos por qualquer instância de Segurança de documentos, desde que você tenha autorização para abrir o arquivo.

Se o servidor de Segurança de Documento utilizar autenticação baseada em certificado, será necessário instalar o certificado recebido na máquina local. Você deve escolher a autenticação de certificado e fornecer o certificado que deseja usar para autenticar.

Após definir as configurações de conexão para uma instância de Segurança de documento em um aplicativo do Microsoft® Office, ele é configurado para todo o Word, Excel e PowerPoint.

#### Instalação do certificado do lado do cliente {#install-the-client-side-certificate}

Se for necessário acessar as páginas da Web do Document Security por meio de autenticação de certificado ou autenticação bidirecional, você receberá o certificado que deve ser instalado na máquina local. Você receberá um arquivo de certificado (arquivo .PFX ou .P12) e sua senha.
...
1. Extensão de segurança de documentos para Microsoft® Office 2010 e Office 2013, no **Segurança de documento** guia , selecione **Escolher Servidor**.
...
1. Clique em **Conectar a este servidor**. A Extensão de Segurança de Documento do Microsoft® Office tenta se conectar ao servidor especificado. Dependendo do tipo de autenticação especificado, execute um dos procedimentos a seguir:
...
>[!NOTE]
>
>Se não conseguir se conectar ao servidor, tente abrir as páginas da Web de Segurança de documentos no Internet Explorer. Se você não conseguir se conectar ao servidor usando o Internet Explorer ou se uma caixa de diálogo exibir um aviso sobre o certificado do servidor, a Extensão de Segurança de Documento para o Microsoft® Office não poderá se conectar ao servidor. Entre em contato com o administrador do servidor para obter assistência.

>[!NOTE]
Se não conseguir se conectar à Segurança de documento, será exibida uma mensagem informando que &quot;O nome de usuário e a senha estão incorretos, verifique as configurações e tente novamente&quot;. Esta mensagem poderá ser exibida se você não conseguir se conectar por outro motivo. Se estiver se conectando ao servidor pela primeira vez, verifique se definiu o nome do servidor e a porta corretamente.

#### Especificação do servidor padrão {#specify-the-default-server}

1. Faça o seguinte:

   * Extensão de segurança de documentos para Microsoft® Office 2010 e Office 2013 no **Segurança de documento** guia , selecione **Escolher Servidor**.
...
Você pode empacotar a página de autenticação personalizada em um arquivo war e implantar o arquivo war no AEM Forms Server. O arquivo war contém uma lógica completa para aceitar credenciais de usuário e autenticar no AEM Forms Server. A Segurança de documentos do AEM Forms tem os seguintes requisitos para a página de autenticação personalizada:
...
Para habilitar a autenticação estendida com um URL de aterrissagem personalizado:

1. Implante o arquivo war de autenticação personalizado no AEM Forms Server.
1. Abra a interface do Administrador do AEM Forms.
1. Navegue até Serviços > Segurança de documentos > Configuração > Configuração do servidor.
...
#### Fluxo de trabalho de autenticação estendida padrão com provedores de identidade de terceiros configurados no AEM Forms Server {#default-extended-authentication-workflow-with-third-party-identity-providers-configured-on-aem-forms-server}

A autenticação estendida pode usar diferentes tipos de autenticação disponíveis no AEM Forms Server. Por exemplo, SAML, [Quais são os outros exemplos].

Observação: Se os provedores de SAML estiverem configurados no AEM Forms Server, antes de exibir o URL de aterrissagem, uma página contendo todos os provedores de identidade configurados para autenticações de SAML será exibida.

A tela a seguir é exibida quando um documento protegido é aberto no Acrobat.

#### Fluxo de trabalho de autenticação estendida personalizado quando provedores SAML são configurados no AEM Forms Server {#custom-extended-authentication-workflow-when-saml-providers-are-configured-on-aem-forms-server}

Se os provedores de SAML estiverem configurados no AEM Forms Server, antes de exibir o URL de aterrissagem, uma página contendo todos os provedores de identidade configurados para autenticações de SAML será exibida.

Os pré-requisitos para configurar um fluxo de trabalho de autenticação estendida personalizado quando os provedores SAML são configurados no AEM Forms Server:

* Autenticações SAML são configuradas no AEM Forms Server
* A guerra personalizada, contendo uma página de autenticação personalizada e uma lógica completa para aceitar credenciais de usuário e autenticar no AEM Forms Server, é implantada no AEM Forms Server.

#### Uso de página personalizada para listar autenticações SAML {#using-custom-page-for-listing-saml-authentications}

Você também pode exibir uma página personalizada para incluir todos os provedores de autenticação configurados no AEM Forms Server. Para criar essa página:

1. Compacte a página de autenticação personalizada em um arquivo war e implante o arquivo war no AEM Forms Server. O arquivo war contém uma lógica completa para aceitar credenciais de usuário e autenticar no AEM Forms Server.
...
1. Extensão de segurança de documentos para o Microsoft® Office 2010 e 2013 no **Segurança de documento** guia , selecione **Seguro > Escolher política**.
...
1. Extensão de segurança de documentos para Microsoft® Office 2010 e 2013, no **Segurança de documento** selecione **Seguro > ***[Nome da política]*.
1. Salve o arquivo.
...
Você pode usar arquivos protegidos por políticas, seja internos ou externos à organização do editor de arquivos. Para abrir arquivos protegidos por política, você deve ser reconhecido pela Segurança de documentos, seja por meio da inclusão em uma lista vinculada do LDAP ou de Diretório ativo, sendo adicionado como um usuário local para o LiveCycle ou o AEM Forms no JEE, ou por meio de registro na Segurança de documentos após ser convidado como usuário.

Se você receber um arquivo protegido por política e não tiver uma conta de Segurança de documentos, ou se receber um convite para se registrar, entre em contato com a pessoa que enviou o arquivo para obter assistência.

### Trabalhar com arquivos protegidos por políticas no Microsoft® Office {#working-with-policy-protected-files-in-microsoft-office}

A extensão de segurança de documentos para o Microsoft® Office restringe determinadas funcionalidades do Word, Excel e PowerPoint para proteger a propriedade intelectual do editor de arquivos. Se você não tiver permissão para alterar o arquivo, não poderá salvar modificações nele.
...
Se um recurso não estiver disponível, o nome do comando no menu e o botão da barra de ferramentas relacionado não estarão disponíveis. Na Extensão de segurança de documentos para o Microsoft® Office, ao passar o ponteiro do mouse sobre o comando ou botão, uma dica de ferramenta indica que o comando está indisponível pela Segurança de documentos.
...
Ao abrir um documento protegido na Extensão Segurança de Documento para o Microsoft® Office a partir do SharePoint Server, certifique-se de que o programa Microsoft® Office associado ao tipo de arquivo, como Microsoft® Word, Microsoft® Excel ou Microsoft® PowerPoint, esteja aberto. Se você tentar abrir o arquivo sem abrir o aplicativo associado, o documento pode não abrir e uma mensagem de erro indicando que você deve instalar o plug-in aplicável é exibida. Além de abrir o aplicativo necessário, é recomendável limpar a pasta de cache antes de abrir um documento protegido na Extensão de Segurança de Documento para o Microsoft® Office a partir do SharePoint Server. Além disso, ao abrir um documento protegido do SharePoint Server, todas as permissões do documento são desativadas, independentemente da política aplicada.
...
1. Extensão de segurança de documentos para Microsoft® Office 2010 e 2013, no **Segurança de documento** guia , selecione **Remover**.
...
Na Extensão de segurança de documentos do Microsoft® Office 2010, o grupo Status de segurança na guia Segurança de documentos exibe suas permissões para o arquivo.

Faça o seguinte:

* Extensão de segurança de documentos para Microsoft® Office 2010 e 2013, no **Guia Segurança de documento** no **Status de segurança** , clique em qualquer item.
...
Se o Administrador tiver ativado a funcionalidade Aplicação automática da política, qualquer documento que você criar ou editar será automaticamente protegido ao salvar o documento.

Se a Aplicação automática da política estiver ativada, a Extensão de segurança de documento para o Microsoft® Office solicitará que você faça logon no servidor de segurança de documentos. Você deve fornecer seu Nome de usuário e Senha para ser autenticado pelo servidor. Se você tiver fornecido as credenciais de logon corretas, o documento será salvo e protegido.
...
* Extensão de segurança de documentos para Microsoft® Office 2010 e 2013, no **Segurança de documento** guia , selecione **Sincronizar offline**.
...
Para abrir as páginas da Web Segurança de documentos, na Extensão de segurança de documentos para o Microsoft® Office 2010 e 2013, na **Segurança de documento** guia , selecione **Criar e gerenciar políticas**. Caso ainda não tenha fornecido as informações de logon, o navegador será aberto na página de logon do servidor.
...
1. Faça o seguinte:
   ...
   * Extensão de segurança de documentos para Microsoft® Office 2010 ou 2013, no **Segurança de documento** guia , selecione **Seguro > Alterar segurança**.
...
1. Faça o seguinte:
   ...
   * Extensão de segurança de documentos para Microsoft® Office 2010 e 2013, no **Segurança de documento** guia , selecione **Revogar**.
...
Extensão de segurança de documentos para Microsoft® Office 2010 e 2013, no **Segurança de documento** guia , selecione **Histórico de auditoria**.
...
>[!NOTE]
Aplicar uma política a um arquivo que contenha um link para um arquivo incorporado não aplica a política ao arquivo vinculado. A Segurança de documento do Microsoft® Office não estende a proteção a arquivos vinculados.
...
* A Segurança de documento do Microsoft® Office não oferece suporte à criação e controle de versão distribuído baseado na Web (WebDAV). Geralmente, não é possível abrir um arquivo protegido por política de uma pasta WebDAV. Se você puder abrir um arquivo protegido por política, não terá permissões para salvar, imprimir, alterar ou copiar do arquivo.

A segurança geral que se aplica a arquivos protegidos por políticas inclui as seguintes restrições:

Muitos recursos comuns podem ser restritos no Word, Excel e PowerPoint durante uma sessão protegida.

Se um arquivo protegido por uma política que não permita ao usuário fazer alterações for aberto, os comandos que alterarem o arquivo de qualquer maneira estarão indisponíveis. Somente os comandos que abrem ou criam documentos e alteram as preferências do aplicativo estão disponíveis.
...
***observação **: A capacidade de iniciar um fluxo de trabalho a partir das versões 2010 do sistema Microsoft® Office do Word, Excel e PowerPoint está disponível apenas no Office Professional Plus 2010, Office Enterprise 2010 e Office Ultimate 2010 suites, bem como nas versões independentes do Office 2010 desses programas.*
...
Para obter informações sobre LiveCycle e formulários AEM na terminologia JEE, consulte [Capítulo 19: Glossário](https://www.adobe.com/go/learn_aemforms_designer_65).
